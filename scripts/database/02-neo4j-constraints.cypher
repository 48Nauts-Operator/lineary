// BETTY Memory System - Neo4j Constraints and Indexes
...
// - Supports bi-temporal relationships (valid time + system time)
// - Enables entity extraction and relationship detection
// - Provides cross-project pattern recognition
// - Maintains temporal evolution of knowledge
...
// =============================================================================
// TEMPORAL CONSTRAINTS FOR GRAPHITI BI-TEMPORAL SUPPORT
// =============================================================================

// Ensure temporal validity constraints
CREATE CONSTRAINT temporal_validity IF NOT EXISTS
FOR (n:Knowledge) REQUIRE n.valid_from IS NOT NULL;

CREATE CONSTRAINT system_time_validity IF NOT EXISTS
FOR (n:Knowledge) REQUIRE n.system_time_from IS NOT NULL;
...
CREATE INDEX knowledge_valid_time_idx IF NOT EXISTS FOR (k:Knowledge) ON (k.valid_from, k.valid_until);
CREATE INDEX knowledge_system_time_idx IF NOT EXISTS FOR (k:Knowledge) ON (k.system_time_from, k.system_time_until);
...
// Core relationship indexes for fast graph traversal
CREATE INDEX rel_created_at_idx IF NOT EXISTS FOR ()-[r]-() ON (r.created_at);
CREATE INDEX rel_strength_idx IF NOT EXISTS FOR ()-[r]-() ON (r.strength);
CREATE INDEX rel_confidence_idx IF NOT EXISTS FOR ()-[r]-() ON (r.confidence);
CREATE INDEX rel_temporal_validity_idx IF NOT EXISTS FOR ()-[r]-() ON (r.valid_from, r.valid_until);
...
// =============================================================================
// GRAPHITI TEMPORAL PATTERNS AND PROCEDURES
// =============================================================================

// Create procedure for temporal knowledge queries
CREATE OR REPLACE PROCEDURE getTemporalKnowledge(
    project_uuid STRING,
    domain STRING,
    as_of_time DATETIME
)
YIELD node, relationship, path
CALL {
    MATCH (p:Project {uuid: $project_uuid})
    MATCH (k:Knowledge)-[:BELONGS_TO]->(p)
    WHERE k.domain = $domain
      AND k.valid_from <= $as_of_time
      AND (k.valid_until IS NULL OR k.valid_until > $as_of_time)
      AND k.system_time_from <= $as_of_time  
      AND (k.system_time_until IS NULL OR k.system_time_until > $as_of_time)
    RETURN k as node, null as relationship, null as path
    
    UNION
    
    MATCH (k1:Knowledge)-[r]->(k2:Knowledge)
    WHERE k1.domain = $domain OR k2.domain = $domain
      AND r.valid_from <= $as_of_time
      AND (r.valid_until IS NULL OR r.valid_until > $as_of_time)
    RETURN null as node, r as relationship, null as path
}
RETURN node, relationship, path;

// Create procedure for cross-project similarity detection
CREATE OR REPLACE PROCEDURE findSimilarKnowledge(
    knowledge_uuid STRING,
    similarity_threshold FLOAT,
    exclude_project_uuid STRING
)
YIELD similar_knowledge, similarity_score
CALL {
    MATCH (source:Knowledge {uuid: $knowledge_uuid})
    MATCH (target:Knowledge)
    WHERE target.project_uuid <> $exclude_project_uuid
      AND target.domain = source.domain
      AND target.system_time_until IS NULL
    
    // Similarity calculation based on multiple factors
    WITH source, target,
         // Technology overlap
         size([t IN source.technologies WHERE t IN target.technologies]) * 1.0 / 
         size(source.technologies + [t IN target.technologies WHERE NOT t IN source.technologies]) as tech_similarity,
         
         // Pattern overlap  
         size([p IN source.patterns WHERE p IN target.patterns]) * 1.0 /
         size(source.patterns + [p IN target.patterns WHERE NOT p IN source.patterns]) as pattern_similarity,
         
         // Quality and reusability scores
         abs(source.quality_score - target.quality_score) as quality_diff,
         abs(source.reusability_score - target.reusability_score) as reusability_diff
    
    WITH source, target,
         (tech_similarity * 0.4 + pattern_similarity * 0.4 + 
          (1.0 - quality_diff) * 0.1 + (1.0 - reusability_diff) * 0.1) as similarity_score
    
    WHERE similarity_score >= $similarity_threshold
    
    RETURN target as similar_knowledge, similarity_score
    ORDER BY similarity_score DESC
    LIMIT 10
}
RETURN similar_knowledge, similarity_score;

// =============================================================================
// MONITORING AND MAINTENANCE PROCEDURES
// =============================================================================

// Procedure to collect graph statistics
CREATE OR REPLACE PROCEDURE getGraphStats()
YIELD metric_name, metric_value
CALL {
    // Node counts
    MATCH (n) 
    WITH labels(n)[0] as node_type, count(n) as node_count
    RETURN node_type + "_count" as metric_name, node_count as metric_value
    ...
    RETURN rel_type + "_count" as metric_name, rel_count as metric_value
    
    UNION
    
    // Knowledge quality metrics
    MATCH (k:Knowledge)
    WHERE k.system_time_until IS NULL
    RETURN "avg_knowledge_quality" as metric_name, avg(k.quality_score) as metric_value
    
    UNION
    
    MATCH (k:Knowledge) 
    WHERE k.system_time_until IS NULL
    RETURN "avg_knowledge_reusability" as metric_name, avg(k.reusability_score) as metric_value
    
    UNION
    
    // Cross-project relationship density
    MATCH (k1:Knowledge)-[r]->(k2:Knowledge)
    WHERE k1.project_uuid <> k2.project_uuid
    WITH count(r) as cross_project_rels
    MATCH ()-[r]->()
    RETURN "cross_project_relationship_ratio" as metric_name, 
           cross_project_rels * 1.0 / count(r) as metric_value
}
RETURN metric_name, metric_value;

// Procedure for temporal cleanup of old knowledge versions
CREATE OR REPLACE PROCEDURE cleanupTemporalVersions(retention_days INTEGER)
YIELD deleted_nodes, deleted_relationships
CALL {
    MATCH (n)
    WHERE n.system_time_until IS NOT NULL 
      AND n.system_time_until < datetime() - duration({days: $retention_days})
    
    WITH collect(n) as nodes_to_delete
    
    MATCH (n)-[r]-()
    WHERE n IN nodes_to_delete
    DELETE r
    
    WITH nodes_to_delete, count(r) as deleted_rels
    
    UNWIND nodes_to_delete as n
    DELETE n
    
    RETURN count(nodes_to_delete) as deleted_nodes, deleted_rels as deleted_relationships
}
